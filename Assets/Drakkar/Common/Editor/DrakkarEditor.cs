...
namespace Drakkar
{
	public static class DrakkarEditor
	{
		...
		#region STYLES
		private static GUIStyle rightAlignLabel,rightAlignementBoldLabel,centered_Label,richHelpBox,textAreaWrap;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static void ArrayGUI<T>(ref T[] arr,string name,ref bool isExpanded,Action<T,int> inspector,Comparison<T> sort) where T : new()
		{
			ArrayGUI<T>(ref arr,name,ref isExpanded,inspector);
			EditorGUILayout.BeginHorizontal();
				GUILayout.FlexibleSpace();
				GUI.contentColor=Color.yellow;
				if (GUILayout.Button("Sort",EditorStyles.miniButton,GUILayout.MaxWidth(66)))
		...
		public static void OverlappingLabel(string label,Color col)
		{
			GUI.contentColor=col;
			EditorGUI.LabelField(RectHorizontalPadding(GUILayoutUtility.GetLastRect(),5),label,rightAlignementStyleLabel);
			GUI.contentColor=Color.white;
		}
		#endregion
		#region UTILITIES
		...
		public static void NameAndReasonWarning(string name,string reason) => Debug.LogWarning("<color=yellow>"+name+" </color><color=red>"+reason+"!</color>");
		#endregion
